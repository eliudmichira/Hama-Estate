...
import { auth, signInWithEmail, signUpWithEmail, signInWithGoogle, signOutUser, onAuthStateChange } from '../firebase';
...
export const useAuth = () => {
  ...
  if (!context) {
    return {
      ...
      savedSearches: [],
      userPreferences: {},
      signUp: () => Promise.resolve({ success: false, error: 'Auth not available' }),
      signIn: () => Promise.resolve({ success: false, error: 'Auth not available' }),
      signInWithGoogleAuth: () => Promise.resolve({ success: false, error: 'Auth not available' }),
...
export const AuthProvider = ({ children }) => {
  ...
  const [contextValue, setContextValue] = useState({
    ...
    signUp: () => Promise.resolve({ success: false, error: 'Auth not initialized' }),
    signIn: () => Promise.resolve({ success: false, error: 'Auth not initialized' }),
    signInWithGoogleAuth: () => Promise.resolve({ success: false, error: 'Auth not initialized' }),
    signOut: () => Promise.resolve(),
    addToFavorites: () => {},
    removeFromFavorites: () => {},
    toggleFavorite: () => {},
  ...
  const loadUserPreferences = async (userId) => {
    try {
      ...
      const response = await fetch(`http://localhost:8800/api/users/${userId}/preferences`, {
        method: 'GET',
        headers: {
  ...
  const saveUserDataToBackend = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8800/api/users/${currentUser.id}/preferences`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      // Silently fail - data is already saved to localStorage
      console.log('Backend not available for saving, using localStorage:', error.message);
    }
  };
  ...
  useEffect(() => {
    localStorage.setItem('estate_saved_searches', JSON.stringify(savedSearches));
  }, [savedSearches]);
  ...
  const getUserRole = () => {
    ...
    if (currentUser.email === 'admin@bogani.com') {
      return 'admin';
    }
    
    // Grant admin access to the current user "Mich Michira" for development/testing
    if (currentUser.name === 'Mich Michira' || currentUser.username === 'Mich Michira') {
      return 'admin';
    }
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      // Check if user email matches admin email
      if (currentUser.email === 'admin@bogani.com' && currentUser.role !== 'admin') {
        newRole = 'admin';
        shouldUpdate = true;
      }
      
      // Grant admin access to the current user "Mich Michira" for development/testing
      if ((currentUser.name === 'Mich Michira' || currentUser.username === 'Mich Michira') && currentUser.role !== 'admin') {
        newRole = 'admin';
        shouldUpdate = true;
      }
  ...
  const createContextValue = () => {
    const newContextValue = {
      ...
      savedSearches,
      userPreferences,
      signUp: signUp || (() => Promise.resolve({ success: false, error: 'Auth not initialized' })),
      signIn: signIn || (() => Promise.resolve({ success: false, error: 'Auth not initialized' })),
      signInWithGoogleAuth: signInWithGoogleAuth || (() => Promise.resolve({ success: false, error: 'Auth not initialized' })),
